...
const VoteSchema = new Schema({
    regNo: {
        ...
        validate: {
            validator: async function (regNo) {
                let votePin = (await VoterModel.findOne({ regNo }, " votePin ")).votePin
    ...
    votePin: {
        type: Number, unique: true, required: true, minLength: 9, maxLength: 9,
        validate: {
    ...
    createdAt: { type: Date, default: new Date(new Date().toLocaleString('en', { timeZone: 'Africa/Lagos' })) }
})
...
const VoterSchema = new Schema({
    ...
    votePin: { type: Number, unique: true, required: true, minLength: 9, maxLength: 9 },
...
async function validCandidate(candidate) {
    if (!await CandidateModel.findOne({ nickname: candidate }))
        return false
    return true
}
...
async function validRegNo(regNo, votePin) {
    if (!await VoterModel.findOne({ regNo, votePin }))
        return false
    return true
}
...
async function validVotePin(votePin, regNo) {
    if (!await VoterModel.findOne({ votePin, regNo }))
        return false
    return true
}